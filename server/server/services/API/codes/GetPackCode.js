...
class GetPackCode extends Base {
  constructor(args) {
    super(args);
  }

  async execute(data) {
    const { site, pub_id, publisher, codes, type } = data.body;
    const siteName = site.replace(/ /g, '_') || '';
    const hash = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 6);
  ...
  function nodeToString(node) {
    ...
    const filepath = `${ __dirname }/src/adwmg-${ publisher.replace(/ /g, '_') }-${ siteName ? siteName + '_' + hash : hash }-pack.js`;
    await fs.promises.writeFile(filepath, template, 'utf-8');
    const link = await cdn.uploadToCDN(filepath);
    await codeModel.create({ link, settings: codes, pub_id, site, type })
