...
import { updateVersionCheckVersionExistMiddleware } from '../../../../server/providers/version/middlewares';

describe('Version => UpdateVersionCheckVersionExistMiddleware', function () {
  ...
  it('should exist version', async () => {
    ...
    await updateVersionCheckVersionExistMiddleware.use(request, response, next);
  ...
  it('should not exist version', async () => {
    ...
    expect(result.message).to.equal(`Update version ${ version } is not exist`);
