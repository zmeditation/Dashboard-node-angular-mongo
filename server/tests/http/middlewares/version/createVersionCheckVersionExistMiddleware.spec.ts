...
import { createVersionCheckVersionExistMiddleware } from '../../../../server/providers/version/middlewares';

describe('Version => CreateVersionCheckVersionExistMiddleware', function () {
  ...
  it('should not exist version', async () => {
    ...
    await createVersionCheckVersionExistMiddleware.use(request, response, next);
  ...
  it('should exist version', async () => {
    ...
    expect(statusResult).to.equal(409);
    expect(send.calledOnce).to.be.true;
    expect(result.message).to.equal(`Version ${ version } already exist`);
