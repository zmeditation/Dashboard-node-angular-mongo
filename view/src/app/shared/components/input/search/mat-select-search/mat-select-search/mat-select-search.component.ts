 ...
 *   template: `
 *     <mat-form-field>
 *       <mat-select [formControl]="pubCtrl" placeholder="Bank">
 *         <mat-select-search [formControl]="bankFilterCtrl"></mat-select-search>
 *         <mat-option *ngFor="let bank of filteredPubs | async" [value]="bank.id">
 *           {{bank.name}}
 *         </mat-option>
 *       </mat-select>
 *     </mat-form-field>
 ...
 *   public pubCtrl: FormControl = new FormControl();
 ...
 *   public filteredPubs: ReplaySubject<Bank[]> = new ReplaySubject<Bank[]>(1);
 *
 ...
 *     this.filteredPubs.next(this.banks.slice());
...
export class MatSelectSearchComponent implements OnInit, OnDestroy, AfterViewInit, ControlValueAccessor {
  ...
  private setOverlayClass() {
    if (this.overlayClassSet) { return; }
  ...
  private initMultipleHandling() {
    ...
    this.matSelect.valueChange.pipe(takeUntil(this._onDestroy)).subscribe((values) => {
      if (this.matSelect.multiple) {
        ...
        if (restoreSelectedValues) { this.matSelect._onChange(values); }
